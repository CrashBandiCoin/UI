...
import {useMatchingRewardLengthTeasport, useWinningNumbersTeasport} from "../../../hooks/useTicketsTeasport";

const WinningNumbers: React.FC = () => {
  const { account } = useWallet()
  const winNumbers = useWinningNumbersTeasport()
  const lotteryHasDrawn = useGetLotteryHasDrawnTeasport()
  const MatchedNumber4 = useMatchingRewardLengthTeasport(4)
  const MatchedNumber3 = useMatchingRewardLengthTeasport(3)
  const MatchedNumber2 = useMatchingRewardLengthTeasport(2)
...
const TicketNumberBox = styled.div`
  padding: 10px;
  border-radius: 12px;
  background: linear-gradient(180deg, #FEA6B9 0%, #FEA6B9 76.22%);
  color: white;
  font-size: 20px;
  font-weight: 900;
  margin: 10px;
